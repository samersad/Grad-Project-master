...
const upload = asyncHandler(async (req, res) => {
  ...
  const rootFolder = bucketFolders[bucket];
  if (!rootFolder) throw new ApiError(400, 'Invalid upload bucket');

  const folder = [rootFolder, req.body.folder].filter(Boolean).join('/');
